	...
	*/
	...
	public function exec()
	{
		try
		{
			...
			LengowCore::log('Export - '.count($products).' product'.(count($products) > 1? 's': '').' found', !$this->stream);
	...
	public function export($products, $fields, $shop)
	{
		...
		foreach ($products as $p)
		{
			...
			foreach ($fields as $field)
			{
				if (isset(LengowExport::$DEFAULT_FIELDS[$field]))
					$product_data[$field] = $product->getData(LengowExport::$DEFAULT_FIELDS[$field], null, $this->full_title);
				else
				...
				if ($this->export_combinations && $product->hasAttributes())
				{
					...
					{
						if (!$this->export_out_stock && $product->getData('quantity', $combination['id_product_attribute'] ) <= 0)
							continue;
						$key = $product->id.'_'.$combination['id_product_attribute'];
						if (isset(LengowExport::$DEFAULT_FIELDS[$field]))
							$combinations_data[$key][$field] = $product->getData(LengowExport::$DEFAULT_FIELDS[$field], $combination['id_product_attribute'], $this->full_title);
						else
							$combinations_data[$key][$field] = $product->getData($field, $combination['id_product_attribute'], $this->full_title);
			...
			if ($this->export_timeout && time() > Configuration::get('LENGOW_EXPORT_END_'.Context::getContext()->language->iso_code))
			{
				...
				LengowCore::log('Export - stopped by timeout. '.$product_count.' products exported.', !$this->stream);
				return;
			}
			$is_first = false;
		}
		...
		if (!$success)
			throw new LengowFileException('Export file generation did not end properly. Please make sure the export folder is writeable.', true);
		if (!$this->stream)
		{
