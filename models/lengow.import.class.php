...
class LengowImport
{
    ...
    protected static function importOrders(
        ...
        foreach ($orders as $order_data) {
            ...
            if (count($order_data->packages) == 0) {
                LengowCore::log('create order fail: no package in the order', true, $lengow_id);
                continue;
            }
            // check order status
            ...
            foreach ($order_data->packages as $package) {
                ...
                if (count($package->cart) == 0) {
                    LengowCore::log('create order fail: no product in the order', $log_output, $lengow_id);
                    continue;
                }
                // if marketplace order line id is null
                if (is_null($package->cart[0]->marketplace_order_line_id)) {
                    LengowCore::log('create order fail: no order line id for a product', $log_output, $lengow_id);
                    continue;
                }
                // get all lines ids
                ...
                if ($order_id) {
                    ...
                    if ($order->is_disabled) {
                        LengowCore::log('order is disabled (ORDER '.$order_id.')', $log_output, $lengow_id);
                        ...
                        try {
                            if ($order->updateState(
                                ...
                                foreach ($available_states as $state) {
                                    if ($state['id_order_state'] === LengowCore::getOrderState($api_state)) {
                ...
                try {
                    ...
                    if (!Configuration::get('LENGOW_IMPORT_PROCESSING_FEE') || $first == false) {
                        ...
                        LengowCore::log('rewrite amount without processing fee', $log_output, $lengow_id);
                    }
                    if ($first == false) {
                        $order_data->commission = 0;
                        $order_data->shipping = 0;
                        LengowCore::log('rewrite amount without shipping cost', $log_output, $lengow_id);
                    }
                    ...
                    if (empty($order_list)) {
                        ...
                        foreach ($order_list as $order) {
                            ...
                            LengowCore::log('save order lines product : '.$order_line_saved, $debug, $lengow_id);
                        ...
                        if (!$debug) {
                            ...
                            if (count($order_ids) > 0) {
                                ...
                                $connector  = new LengowConnector(
                                    LengowCore::getAccessToken(),
                                    LengowCore::getSecretCustomer()
                                );
                        ...
                        try {
                            ...
                            LengowCore::log($lce->getMessage(), $debug, $lengow_id);
                        }
                    }
                    if ($shipped_by_mp) {
                        LengowCore::log(
    ...
    protected static function checkOrderData($order_data, $package, $lengow_id, $order_line_id, $debug)
    {
        ...
        if ($error_message) {
            LengowCore::log('order import failed: '.$error_message, true, $lengow_id);
            if (!$debug) {
