...
class LengowOrder extends Order
{
    ...
    public static function getCustomerEmailByMarketplaceCustomerId(
        ...
        // no LIMIT clause in this query: Db::getRow() always appends its own ' LIMIT 1',
        // which would produce an invalid 'LIMIT 1 LIMIT 1' statement
    ...
    /**
     * Check whether an existing PrestaShop customer already belongs to another marketplace buyer
     *
     * Looks for a Lengow order attached to this customer carrying a different, non empty
     * marketplace_customer_id: this is the only positive proof that the email is shared by
     * several end customers. Without such proof, the customer must be considered as the same
     * buyer placing a new order.
     *
