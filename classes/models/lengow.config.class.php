...
class LengowConfig
{
    ...
    public function postProcessForm()
    {
        ...
        if (Tools::getIsset('submitlengow')) {
            ...
            LengowConfiguration::updateGlobalValue('LENGOW_IMPORT_FORCE_PRODUCT', Tools::getValue('lengow_import_force_product'));
            LengowConfiguration::updateGlobalValue('LENGOW_IMPORT_DAYS', Tools::getValue('lengow_import_days'));
            ...
            LengowConfiguration::updateGlobalValue('LENGOW_DEBUG', Tools::getValue('lengow_debug'));
    ...
    private function getHelpSolutionIds()
    {
        ...
        $out .= sprintf($this->module->l('You can find credentials on %s.'),
            '<a href="https://solution.lengow.com/api/" target="_blank">' . $this->module->l('your Lengow Dashboard') . '</a>');
        $out .= '<br />';
        $out .= $this->module->l('You can add more than 1 group, must be separated by <b>,</b>');
        $out .= '<br />';
        $out .= sprintf($this->module->l('Make sure your website IP (%s) address is filled in your Lengow Dashboard.',
            'lengow.check.class'), $_SERVER['REMOTE_ADDR']);
        $out .= '<br />';
        $out .= sprintf($this->module->l('%s for assistance.'),
            '<a href="' . $this->module->l('https://en.helpgizmo.com/help/article/link/prestashopv2') . '" target="_blank">' . $this->module->l('Click here') . '</a>');
        $out .= '</p>';
        return $out;
    }
