...
class LengowMain
{
    ...
    public static function getOrderState(string $state): int|false
    {
        switch ($state) {
            ...
            case LengowOrder::STATE_REFUNDED:
                return (int) LengowConfiguration::getGlobalValue(LengowConfiguration::REFUNDED_ORDER_ID);
            case LengowOrder::STATE_PARTIALLY_REFUNDED:
                return (int) LengowConfiguration::getGlobalValue(LengowConfiguration::PARTIAL_REFUNDED_ORDER_ID);
     ...
     * MondialrelayClasslib namespace) as v3.x, so it is handled by the same
     * LengowCarrier::handleMRRelayV3() code path.
     *
     * @return bool
     */
    public static function isMondialRelayV4Available(): bool
    {
    ...
    /**
     * Check if the Colissimo module (colissimo v2+/v3, external_module_name = 'colissimo') is installed,
     * activated, and version is supported
     *
     * @return bool
     */
    public static function isColissimoAvailable(): bool
    {
        ...
        require_once $moduleDir . $moduleName . '.php';
        if (!class_exists('Colissimo')) {
            return false;
        }
        $colissimo = new Colissimo();

        return version_compare((string) $colissimo->version, $supportedVersion, '>=');
    }
